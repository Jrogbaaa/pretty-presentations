 ...
 * Uses client SDK (no service account needed)
 */
 ...
 */
function parseCategories(genre) {
...
function transformInfluencer(row, index) {
  ...
  if (!name) return null; // Skip rows without names
  
  const handle = parseHandle(row['Instagram Handle'] || row['instagram_handle'] || '');
  if (!handle) return null; // Skip rows without handles
  
  const followers = parseFollowers(row.Followers || row['followers'] || '0');
  const engagement = parseEngagement(row['Engagement Rate'] || row['engagement_rate'] || '0');
  const categories = parseCategories(row.GENRE || row['genre'] || '');
  ...
  return {
    ...
    performance: {
      ...
      contentQualityScore: Math.min(10, 5 + (engagement / 2))
    },
...
async function importInfluencers() {
  ...
  try {
    ...
    const allInfluencers = records
      .map((row, index) => transformInfluencer(row, index))
      .filter(inf => inf !== null); // Remove invalid entries
    
    // Filter out duplicates and invalid entries
    const newInfluencers = allInfluencers.filter(inf => {
      const handle = inf.handle.toLowerCase();
      return handle && handle.length > 0 && !existingHandles.has(handle);
    });
    ...
    // Import in batches (Firestore batch limit is 500)
    ...
    for (let i = 0; i < newInfluencers.length; i += BATCH_SIZE) {
      ...
      chunk.forEach(influencer => {
        try {
          const docRef = doc(db, 'influencers', influencer.id);
    ...
    const finalCount = verifySnapshot.size;
